...
exports.startOwner = (app) => {
    ...
    app.get('/Owner/ItemList', (req, res) => {
        ...
        if (user.userID) {
            ...
            sql.getOwnerCnt(user.userID, null, null, (table) => {
                sql.getOwnerCntbyDate(user.userID, getDate(), (today) => {
                    console.log(today);
                    var ItemCnt=0;
                    if(today[0])  {
                       ItemCnt=today[0].ItemCnt; 
                    }
                    sql.getOnwerTimeline(user.userID, getDate(),yyyymm+'-01', end, (routeTimeline)=>{
                        res.render('./Owner/itemList', { user: user, yyyymm: yyyymm, table: table, ItemCnt:ItemCnt, routeTimeline:routeTimeline });
